...
/****** Object:  StoredProcedure [dbo].[spr_GetSAPBillingQuotesInformation]    Script Date: 16/07/2014 02:13:45 p.m. ******/
SET ANSI_NULLS ON
GO
...
* Description: Leer datos de cuotas  por dato de facturacion
***************************************************************/
CREATE PROCEDURE [dbo].[spr_GetSAPBillingQuotesInformation]
	@LimitDate varchar(10),
	@IdBilling varchar(20)
AS

	DECLARE @oLimitDate varchar(10) = @LimitDate 
	DECLARE @oIdBilling varchar(20) = @IdBilling 
...
select 
	  ...
	  cuotasFacturacion.[ESTADO__C] as quoteStatus,
	  cuotasFacturacion.[ID] as quoteId
	  --MasterData.CODCIUDAD as MDcodciudad,
	  --MasterData.CORREO as MDCorreo,
	  --MasterData.DIRECCION as MDDireccion,
	  --MasterData.CIUDAD as MDCiudad,
	  --MasterData.TELEFONO as MDTelefono
	  --into [dbo].[Z_logFactSFSAPsoldig]
   ...
   inner join ( SELECT case when productosCompradoOp.productosComprados = activosOportunidad.activosActivos then productosCompradoOp.ID end COLLATE Latin1_General_CS_AS as ID from
					...
					inner join
					  ( select count(activos.ID) as activosActivos, op.ID  COLLATE Latin1_General_CS_AS AS ID
					   ...
					   inner join dbo.QUOTELINEITEM productosCotizacion  WITH(NOLOCK) on productosCotizacion.QUOTEID = op.SYNCEDQUOTEID
					   inner join dbo.ASSET activos  WITH(NOLOCK) on activos.ID = productosCotizacion.ACTIVO_PRODUCIDO__C
					   where activos.[status] in ('03. Activo','04. Desactivado', '05. Pausado') and activos.INSTALLDATE > '2014-01-01'
   ...
   where StageName = '09 Cerrada ganada' and 
      cotizacion.ISSYNCING = 1 
	 ...
	 and datosFActuracion.TELEFONO_CONTACTO_FINANCIERO__C is not null 
	  ...
	  and datosFacturacion.[DIRECCION_FACTURACION__C] is not null
	  and datosFacturacion.[REQUIERE_MASTERDATA__C] = 0
	  and datosFacturacion.id = @oIdBilling
	  and  datosFacturacion.FECHA_PRIMERA_CUOTA__C <= @oLimitDate
	  and cuotasFacturacion.[ESTADO__C]  = 'N'
